...
class jwst_photclass(pdastrostatsclass):
    ...
    def expand_mask(self, mask, size = 40, mask_shape = 'square'):
        """
    ...
    def add_bin_dq(self, filename):
        ...
        expmask = self.expand_mask(dq_mask, mask_shape = 'circle')
    ...
    def load_image(self, imagename, imagetype=None, DNunits=False, use_dq=False, bin_dq = False, skip_preparing=False):
        ...
        if not skip_preparing:
            ...
            if self.imagetype == 'cal':
                ...
                if use_dq: 
                    if bin_dq:
                        if 'BIN_DQ' not in self.im:
                            self.add_bin_dq(imagename)
                        dq = self.im['BIN_DQ'].data
                    else:
                        dq = self.im['DQ'].data
...
#                nan_mask = np.where(zero_mask == 0,True,False)
    ...
    def xy_to_radec(self,xcol='x',ycol='y',racol='ra',deccol='dec',indices=None,
        ...
        # numpy arrays avoid pandas Series.mask conflicting with Astropy masked arrays
        xpix = np.asarray(self.t.loc[ixs, xcol] + xshift, dtype=float)
        ypix = np.asarray(self.t.loc[ixs, ycol] + yshift, dtype=float)
        coord = self.sci_wcs.pixel_to_world(xpix, ypix)
    ...
    def match_refcat(self,
        ...
        self.refcat.t['x'], self.refcat.t['y'] = self.sci_wcs.world_to_pixel(
            SkyCoord(
                np.asarray(self.refcat.t[self.refcat.racol], dtype=float),
                np.asarray(self.refcat.t[self.refcat.deccol], dtype=float),
                unit=u.deg,
            )
        )
        ...
        # Get the detector x,y position
        self.refcat.t.loc[ixs_refcat, 'x'], self.refcat.t.loc[ixs_refcat, 'y'] = (
            self.sci_wcs.world_to_pixel(
                SkyCoord(
                    np.asarray(self.refcat.t.loc[ixs_refcat, self.refcat.racol], dtype=float),
                    np.asarray(self.refcat.t.loc[ixs_refcat, self.refcat.deccol], dtype=float),
                    unit=u.deg,
                )
            )
        )

        refcatcoord = SkyCoord(
            np.asarray(self.refcat.t.loc[ixs_refcat, self.refcat.racol], dtype=float),
            np.asarray(self.refcat.t.loc[ixs_refcat, self.refcat.deccol], dtype=float),
            unit='deg',
        )
