...
class pdastroclass:
    ...
    def load(self,filename,raiseError=True,test4commentedheader=False,namesMapping=None,roundingMapping=None,
        ...
        try:
            ...
            # pandas>=2.2 / 3.x removed delim_whitespace; sep=r'\s+' is the replacement.
            read_kwargs = dict(kwargs)
            if delim_whitespace and 'sep' not in read_kwargs and 'delimiter' not in read_kwargs:
                read_kwargs['sep'] = r'\s+'
            if check4csv and re.search('csv$', filename):
                self.t = pd.read_csv(filename, comment='#', **read_kwargs)
            else:
                self.t = pd.read_table(filename, **read_kwargs)
