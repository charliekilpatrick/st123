...
#            deccol = f'{refcat_short}_dec'
    ...
    def update_phottable_final_wcs(self,outputfits,
        ...
        if self.telescope.lower()=='hst' and self.phot.do_driz:
            ...
            #datamodels ImageModel seems to fail for i2d images
            image_model = fits.open(outputfits)
            image_wcs = wcs.WCS(image_model['SCI'].header)
            # Convert pandas Series -> ndarray: Astropy>=6/8 confuses Series.mask
            # with masked-array handling in pixel_to_world / world_to_pixel.
            ra_arr = np.asarray(phot.t[refcat_racol], dtype=float)
            dec_arr = np.asarray(phot.t[refcat_deccol], dtype=float)
            x_arr = np.asarray(phot.t['x'], dtype=float)
            y_arr = np.asarray(phot.t['y'], dtype=float)
            phot.t[refcat_xcol], phot.t[refcat_ycol] = image_wcs.world_to_pixel(
                SkyCoord(ra_arr, dec_arr, unit=u.deg)
            )
            sc = image_wcs.pixel_to_world(x_arr, y_arr)
            phot.t['ra'] = sc.ra.value
            phot.t['dec'] = sc.dec.value
